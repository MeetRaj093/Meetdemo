...
class WCVendors_Pro {
	...
	public function __construct() {
		...
		$this->wcvendors_pro_import_export          = new WCVendors_Pro_Import_Export( $this->get_plugin_name(), $this->get_version(), $this->get_debug() );
		$this->wcvendors_pro_product_reports_form   = new WCVendors_Pro_Product_Reports_Form();
		$this->wcvendors_pro_product_reports        = new WCVendors_Pro_Admin_Product_Reports();
		...
		$this->wcvendors_pro_product_reports_controller = new WCVendors_Pro_Product_Reports_Controller();
	...
	private function load_dependencies() {
		...
		include_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-wcvendors-pro-admin-product-reports.php';
		...
		include_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-wcvendors-pro-product-reports-controller.php';
		include_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-wcvendors-pro-product-reports-table.php';
		...
		include_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-wcvendors-pro-import-export.php';
		...
		include_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/forms/class-wcvendors-product-reports-form.php';
	...
	private function define_admin_hooks() {
		...
		// Update commission status after refunded.
		$this->loader->add_action( 'woocommerce_order_refunded', $this->wcvendors_pro_commission_controller, 'reversed_refunded_status', 10, 1 );
	...
	private function define_public_hooks() {
		...
		$markreceived_disabled         = get_option( 'wcvendors_hide_settings_mark_received', 'yes' );
		...
		$this->loader->add_action( $this->prefix . 'after_tax_field', $this->wcvendors_pro_public, 'show_dimension_when_shipping_disabled', 10, 1 );
		$this->loader->add_action( 'woocommerce_product_meta_end', $this->wcvendors_pro_product_reports_form, 'product_reports_form_content', 10, 1 );
		...
		$this->loader->add_filter( 'wcv_dashboard_custom_pages', $this->wcvendors_pro_import_export, 'add_subpages', 9 );
		$this->loader->add_action( 'wcv_pro_dashboard_custom_template', $this->wcvendors_pro_import_export, 'add_import_page', 10, 4 );
		$this->loader->add_action( 'wp_ajax_wcv_do_ajax_product_export', $this->wcvendors_pro_import_export, 'do_ajax_product_export' );
		$this->loader->add_action( 'wp_ajax_wcv_do_ajax_product_import', $this->wcvendors_pro_import_export, 'do_ajax_product_import' );
		$this->loader->add_action( 'template_redirect', $this->wcvendors_pro_import_export, 'download_export_file' );
		$this->loader->add_action( 'template_redirect', $this->wcvendors_pro_import_export, 'handle_steps' );
		$this->loader->add_filter( 'wcvendors_product_import_process_item_data', $this->wcvendors_pro_import_export, 'disable_featured_column' );
		$this->loader->add_filter( 'wcvendors_product_import_process_item_data', $this->wcvendors_pro_import_export, 'check_product_owner' );
		// $this->loader->add_filter( 'woocommerce_product_import_pre_insert_product_object', $this->wcvendors_pro_import_export, 'check_product_status', 10, 2 );
		...
		$this->loader->add_filter( 'user_has_cap', $this->wcvendors_pro_vendor_controller, 'allow_vendor_to_edit_own_images', 10, 4 );
		...
		if ( 'free' !== $pro_store_header ) {
			...
			if ( $single_store_header ) {
				$this->loader->add_action( 'woocommerce_before_main_content', $this->wcvendors_pro_vendor_controller, 'store_single_header', $single_product_header_prority );
			} else {
				$this->loader->add_action( 'woocommerce_before_main_content', $this->wcvendors_pro_vendor_controller, 'vacation_mode' );
			}
		}
