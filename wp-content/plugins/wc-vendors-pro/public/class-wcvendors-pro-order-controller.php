...
class WCVendors_Pro_Order_Controller {
	 ...
	 *
	 * @since   1.0.0
	 * @since   1.8.2 - Added extra checks
	 */
	public function process_submit() {

		if ( isset( $_GET['wcv_mark_shipped'] ) ) {

			if ( ! WCVendors_Pro_Dashboard::check_object_permission( 'order', absint( $_GET['wcv_mark_shipped'] ) ) ) {
				return false;
			}
		...
		if ( isset( $_POST['wcv_order_id'] ) && isset( $_POST['wcv_add_note'] ) ) {
			...
			if ( ! WCVendors_Pro_Dashboard::check_object_permission( 'order', absint( $order_id ) ) ) {
				return false;
			}
		...
		if ( isset( $_POST['wcv_add_tracking_number'] ) ) {
			...
			if ( ! WCVendors_Pro_Dashboard::check_object_permission( 'order', absint( $_POST['_wcv_order_id'] ) ) ) {
				return false;
			}
		...
		if ( ! empty( $all_orders ) ) {
			...
			foreach ( $all_orders as $_order ) {
				...
				if ( ! empty( $_order->order_items ) ) {
					$refunded_total = 0;
					$refunded_count = count( $order->get_refunds() );
					$order_total    = 0;
					...
					foreach ( $_order->order_items as $item ) {
						...
						if ( $refunded_count > 0 ) {
							$item_refunded_total = $order->get_total_refunded_for_item( $item_id );

							if ( absint( $item_refunded_total ) > 0 ) {
								$item_name = sprintf( '<del>%s</del>', $item_name );
								$item_tax  = wc_tax_enabled() ? $item->get_taxes() : false;
								if ( $item_tax && $_order->total_tax > 0 ) {
									foreach ( $order->get_taxes() as $tax ) {
										$tax_id                  = $tax['rate_id'];
										$item_tax_refunded_total = $order->get_tax_refunded_for_item( $item_id, $tax_id );
									}
								}
					...
					if ( $refunded_total > 0 ) {
						...
						if ( $refunded_total < $order_total ) {
							$refunded_status = '<span class="wcv-order-refunded">' . __( 'Partially Refunded', 'wcvendors-pro' ) . '</span>';
						} else {
							$refunded_status = '<span class="wcv-order-refunded">' . __( 'Refunded', 'wcvendors-pro' ) . '</span>';
						}
					}
				}
				...
				$price_total    = wc_price( $_order->total );
				if ( $refunded_total > 0 ) {
					$refund_text = __( 'Refunded: ', 'wcvendors-pro' ) . sprintf( get_woocommerce_price_format(), get_woocommerce_currency_symbol( $order_currency ), number_format( $refunded_total, 2 ) );
					$price_total = sprintf( '<del>%s</del> %s', wc_price( $_order->total ), wc_price( $_order->total - $refunded_total ) );
				}

				$total_text = '<span class="wcv-tooltip" data-tip-text="' . sprintf( "%s %s\n %s %s\n %s %s\n %s %s\n %s %s \n %s", __( 'Full Commission: ', 'wcvendors-pro' ), $commission, __( 'Commission Due: ', 'wcvendors-pro' ), $commission_due, __( 'Product: ', 'wcvendors-pro' ), $product_price, __( 'Shipping: ', 'wcvendors-pro' ), $shipping_due, __( 'Tax: ', 'wcvendors-pro' ), $tax_due, $refund_text ) . '">' . $price_total . '</span>';
				...
				$new_row->order_date   = date_i18n( get_option( 'date_format', 'F j, Y' ), ( $order_date->getOffsetTimestamp() ) ) . '<br /><strong>' . ucfirst( wc_get_order_status_name( $order->get_status() ) ) . '</strong><br />' . $refunded_status;
		...
		foreach ( $line_items as $item_id => $item ) {
			...
			if ( $tax_data ) {
				foreach ( $order_taxes as $tax_item ) {
					...
					if ( $is_order_refund && $_order->total_tax > 0 ) {
	 ...
	 */
	public function get_key_value( $array, $key ) {
