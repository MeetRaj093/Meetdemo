...
class WCVendors_Pro_Product_Controller {
	...
	public function process_submit() {
		...
		if ( isset( $_POST['post_id'] ) && is_numeric( $_POST['post_id'] ) ) {
			...
			if ( ! WCVendors_Pro_Dashboard::check_object_permission( 'product', absint( $_POST['post_id'] ) ) ) {
				return;
			}
	...
	public function process_delete() {
		...
		if ( isset( $wp->query_vars['object'] ) ) {
			...
			if ( $object == 'product' && $action == 'delete' && is_numeric( $id ) ) {
				...
				if ( $id != null ) {
					if ( WCVendors_Pro_Dashboard::check_object_permission( 'product', $id ) == false ) {
						return false;
					}
				}
				...
				if ( 'yes' !== get_option( 'wcvendors_vendor_product_trash', 'no' ) ) {
					...
					if ( is_object( $update ) || is_numeric( $update ) ) {
						...
						do_action_deprecated( 'wcvendors_delete_post', array( $id ), '1.8.2', 'wcvendors_product_delete' );
	...
	public function save_meta( $post_id ) {
		...
		if ( 'no' == $is_virtual ) {
			...
			do_action( 'wcvendors_before_product_save_shipping_rates', $product_id );
			// shipping rates
