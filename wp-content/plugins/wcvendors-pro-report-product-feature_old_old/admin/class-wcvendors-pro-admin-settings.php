...
class WCVendors_Pro_Admin_Settings {
	...
	public function forms_page( $settings ) {
		...
		$settings[] = include WCV_PRO_ABSPATH_ADMIN . 'settings/class-wcvendors-pro-settings-report-product.php';
	...
	public function general( $settings ) {
		...
		foreach ( $settings as $setting ) {
			...
			if ( isset( $setting['id'] ) && 'general_options' == $setting['id'] && isset( $setting['type'] ) && 'sectionend' == $setting['type'] ) {
				...
				// Mark Received.
				$new_settings[] = array(
					'title'   => __( 'Mark Received', 'wcvendors-pro' ),
					'desc'    => __( 'Allow customers to mark their orders received.', 'wcvendors-pro' ),
					'id'      => 'wcvendors_hide_settings_mark_received',
					'type'    => 'checkbox',
					'default' => 'yes',
				);
			}
	...
	public function capabilities( $settings ) {
		...
		foreach ( $settings as $setting ) {
			...
			if ( isset( $setting['id'] ) && 'permissions_products_options' == $setting['id'] && isset( $setting['type'] ) && 'sectionend' == $setting['type'] ) {
				...
				$new_settings[] = array(
					'title'   => __( 'Import Products', 'wcvendors-pro' ),
					'desc'    => sprintf( __( 'Allow %s to import products', 'wcvendors-pro' ), wcv_get_vendor_name( false, false ) ),
					'tip'     => sprintf( __( 'Allow %s to import products', 'wcvendors-pro' ), lcfirst( wcv_get_vendor_name( false ) ) ),
					'id'      => 'wcvendors_capability_products_import',
					'type'    => 'checkbox',
					'default' => 'no',
				);

				$new_settings[] = array(
					'title'   => __( 'Export Products', 'wcvendors-pro' ),
					'desc'    => sprintf( __( 'Allow %s to export products', 'wcvendors-pro' ), wcv_get_vendor_name( false, false ) ),
					'tip'     => sprintf( __( 'Allow %s to export products', 'wcvendors-pro' ), lcfirst( wcv_get_vendor_name( false ) ) ),
					'id'      => 'wcvendors_capability_products_export',
					'type'    => 'checkbox',
					'default' => 'no',
				);
		...
		if ( 'pro_dashboard' === $current_section ) {
			...
			$settings = apply_filters(
				...
				array(
					...
					array(
						'title'   => __( 'Vendor single product permalink', 'wcvendors-pro' ),
						'desc'    => __( 'Include vendor store name in single product permalink', 'wcvendors-pro' ),
						'id'      => 'wcvendors_dashboard_single_product_permalink',
						'type'    => 'checkbox',
						'default' => 'yes',
					),
