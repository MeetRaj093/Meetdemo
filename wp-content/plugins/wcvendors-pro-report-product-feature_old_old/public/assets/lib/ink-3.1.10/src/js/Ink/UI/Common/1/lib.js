...
Ink.createModule(
	'Ink.UI.Common',
	'1',
	['Ink.Dom.Element_1', 'Ink.Net.Ajax_1','Ink.Dom.Css_1','Ink.Dom.Selector_1','Ink.Util.Url_1'],
	function(InkElement, Ajax,Css,Selector,Url) {
...
throw new TypeError( 'o should be an object!' ); }
		...
		restoreIdAndClasses: function(toEl, inObj) {
			...
			if (inObj._classes && toEl.className.indexOf( inObj._classes ) === -1) {
				if (toEl.className) {
toEl.className += ' ' + inObj._classes; } else {
...
return; }
			...
			for (var i = 0, len = instances.length; i < len; i++) {
				if (nameWithoutVersion === getName( instances[i] )) {
					...
					Ink.warn(
						'Creating more than one ' + nameWithoutVersion + ' for the same element.',
							'(Was creating a ' + nameWithoutVersion + ' on:',
						elm,
						').'
						);
					return false;
...
throw new TypeError( 'Ink.UI.Common.registerInstance: The element passed in is not a DOM element!' ); }
...
if (prototype.hasOwnProperty( k )) {
	...
	function BaseUIComponent(element, options) {
			...
			if ( ! element && ! constructor._componentOptions.elementIsOptional) {
				...
				this._element = Common.elsOrSelector(
				element,
				_name + ': An element with the selector "' + element + '" was not found!'
				)[0];
				}
	...
	BaseUIComponent._validateInstance = function (instance) {
			...
			if (typeof instance._validate !== 'function') {
	return true; }
	...
	Common.createUIComponent = function createUIComponent(theConstructor, options) {
			...
			function assertProp(prop, propType, message) {
				...
				assert(
					propType && typeof propVal === propType,
					'typeof ' + theConstructor + '.' + prop + ' is not "' + propType + '". ' + message
					);
				}

			assert(
			typeof theConstructor === 'function',
			...
			'_name',
			'string',
			'This property is used for error ' +
