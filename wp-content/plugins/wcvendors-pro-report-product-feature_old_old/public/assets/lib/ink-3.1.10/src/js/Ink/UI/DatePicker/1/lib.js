...
Ink.createModule(
	'Ink.UI.DatePicker',
	'1',
	['Ink.UI.Common_1','Ink.Dom.Event_1','Ink.Dom.Css_1','Ink.Dom.Element_1','Ink.Dom.Selector_1','Ink.Util.Array_1','Ink.Util.Date_1', 'Ink.Util.I18n_1'],
	function(Common, Event, Css, InkElement, Selector, InkArray, InkDate, I18n ) {
	'use strict';
		...
		_render: function() {
			...
			if ( ! this._options.onFocus || this._options.displayInSelect) {
				if ( ! this._options.pickerField) {
					this._picker = InkElement.create(
						'a',
						{
						href: '#open_cal',
						setHTML: 'open',
			...
			this._monthChanger = InkElement.create(
				'a',
				{
				href: '#monthchanger',
			...
			this._ofText = InkElement.create(
				'span',
				{
			...
			this._yearChanger = InkElement.create(
				'a',
				{
				href: '#yearchanger',
...
return noLimits(); }
		...
		_dateCmpUntil: function (self, oth, depth) {
			...
			do {
				i++;
				if (self[props[i]] > oth[props[i]]) {
return 1; } else if (self[props[i]] < oth[props[i]]) {
					return -1; }
			} while (props[i] !== depth &&
		...
		_createSelectOptions: function () {
			...
			InkElement.fillSelect(
				...
				function (monthName, i) { return [ i + 1, monthName ]; }
				),
				true
				);
...
return null; }
		...
		_getPrevDecade: function (date) {
			date       = date || { _year: this._year, _month: this._month, _day: this._day };
			var decade = this._getCurrentDecade( date );
			if (getDecade( decade - 10 ) < getDecade( this._min._year )) {
return null; }
			return decade - 10;
			},
		...
		_addMonthButtonClassNames: function (btn) {
			...
			if ( ! data.calMonth) {
throw 'not a calendar month button!'; }
