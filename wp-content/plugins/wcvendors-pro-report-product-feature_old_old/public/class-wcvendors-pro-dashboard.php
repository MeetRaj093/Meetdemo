...
class WCVendors_Pro_Dashboard {
		...
		if ( 'edit' == $action ) {
			...
			if ( ! empty( $custom ) ) {
				...
				if ( array_key_exists( 'template_name', $custom_pages[ $custom ] ) ) {
					...
					do_action( 'wcv_pro_dashboard_custom_template', $object, $object_id, $template, $custom );
				}
		} else {
	 ...
	 *
	 * @since  1.0.0
	 * @since  1.8.2 - Added extra object check
	 * @since  1.8.3 - Added filters for 3rd party plugins to hook into checks
	 *
	 * @param    string $object the object type to test.
	 * @param    int    $object_id  object id to check.
	 */
	public static function check_object_permission( $object, $object_id ) {
		...
		switch ( $object ) {
			// Product permissions.
			case 'product':
				return ( $can_edit_live && WCV_Vendors::get_vendor_from_product( $object_id ) == get_current_user_id() ) ? true : false;
			case 'shop_coupon':
				return ( WCVendors_Pro_Vendor_Controller::get_vendor_from_object( $object_id ) != get_current_user_id() ) ? false : true;
			case 'order':
