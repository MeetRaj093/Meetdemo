	...
	public function enqueue_styles() {
		...
		if ( wcv_is_dashboard_page( $current_page_id ) || ( $view_feedback ) || ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'wcv_pro_dashboard_nav' ) ) || ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'wcv_pro_dashboard' ) ) ) {
			...
			if ( is_user_logged_in() ) {
				// Dashboard Style.
				wp_enqueue_style( 'wcv-pro-dashboard', apply_filters( 'wcv_pro_dashboard_style', $this->base_dir . 'assets/css/dashboard' . $this->suffix . '.css' ), false, rand() );
			}
	...
	public function enqueue_scripts() {
		...
		if ( wcv_is_dashboard_page( $current_page_id ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				$product_params = array(
					...
					'shipping_system_enabled'        => $shipping_method_enabled,
				);
				...
				wp_register_script( 'wcv-product-export', $this->base_dir . 'assets/js/product-export' . $this->suffix . '.js', array( 'jquery' ), WCV_PRO_VERSION );
				wp_localize_script(
					'wcv-product-export',
				...
				wp_register_script( 'wc-product-import', $this->base_dir . 'assets/js/product-import' . $this->suffix . '.js', array( 'jquery' ), WCV_PRO_VERSION );
		...
		if ( is_product() ) {
			...
			if ( is_user_logged_in() ) {
				...
				wp_register_script( 'wcv-report-product-script', $this->base_dir . 'assets/js/report-product' . $this->suffix . '.js', array( 'jquery' ), WCV_PRO_VERSION, true );
				wp_enqueue_script( 'wcv-report-product-script' );

				wp_register_style( 'wcv-report-product-style', $this->base_dir . 'assets/css/report-product' . $this->suffix . '.css', array(), WCV_PRO_VERSION );
				wp_enqueue_style( 'wcv-report-product-style' );
			}
		}
