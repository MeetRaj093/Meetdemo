...
abstract class WC_Product_Importer implements WC_Importer_Interface {
	...
	protected function get_product_object( $data ) {
		...
		if ( isset( $data['type'] ) ) {
			...
			if ( in_array( $data['type'], $product_type_settings ) ) {
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				...
				throw new Exception( sprintf( __( 'Not able to attach "%s".', 'wcvendors-pro' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			/* translators: %s: image URL */
			throw new Exception( sprintf( __( 'Unable to use image "%s".', 'wcvendors-pro' ), $url ), 400 );
		}
