<?php
/**
 * WC Vendors Product CSV importer
 *
 * @package WCVendors\Import
 * @version 3.1.0
 */
...
/**
 * WCV_Product_CSV_Importer Class.
 */
class WCV_Product_CSV_Importer extends WC_Product_Importer {
	...
	public function __construct( $file, $params = array() ) {
		...
		include_once dirname( dirname( __FILE__ ) ) . '/importers/mappings/mappings.php';
	...
	protected function read_file() {
		if ( ! WCV_Product_CSV_Importer_Controller::is_file_valid_csv( $this->file ) ) {
			wp_die( esc_html__( 'Invalid file type. The importer supports CSV and TXT file formats.', 'wcvendors-pro' ) );
		}
	...
	public function parse_categories_field( $value ) {
		...
		foreach ( $row_terms as $row_term ) {
			...
			foreach ( $_terms as $index => $_term ) {
				// Don't allow vendors create new categories.
				if ( ! term_exists( $_term, 'product_cat', intval( $parent ) ) ) {
					break;
				}
	...
	protected function get_row_id( $parsed_data ) {
		...
		if ( $id ) {
			...
			$row_data[] = sprintf( __( 'ID %d', 'wcvendors-pro' ), $id );
		}
		if ( $sku ) {
			/* translators: %s: product SKU */
			$row_data[] = sprintf( __( 'SKU %s', 'wcvendors-pro' ), $sku );
		}
		...
		foreach ( $this->parsed_data as $parsed_data_key => $parsed_data ) {
			...
			if ( $id_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					...
					esc_html__( 'A product with this ID already exists.', 'wcvendors-pro' ),
					array(
						'id'  => $id,
			...
			if ( $sku_exists && ! $update_existing ) {
				$data['skipped'][] = new WP_Error(
					'woocommerce_product_importer_error',
					esc_html__( 'A product with this SKU already exists.', 'wcvendors-pro' ),
					array(
						'sku' => esc_attr( $sku ),
						'row' => $this->get_row_id( $parsed_data ),
					)
			...
			if ( $update_existing && ( isset( $parsed_data['id'] ) || isset( $parsed_data['sku'] ) ) && ! $id_exists && ! $sku_exists ) {
				$data['skipped'][] = new WP_Error(
					'woocommerce_product_importer_error',
					esc_html__( 'No matching product exists to update.', 'wcvendors-pro' ),
					array(
						'id'  => $id,
						'sku' => esc_attr( $sku ),
						'row' => $this->get_row_id( $parsed_data ),
