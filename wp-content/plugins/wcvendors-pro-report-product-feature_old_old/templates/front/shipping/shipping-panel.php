...
<?php else : ?>
	...
	<?php if ( ! empty( $shipping_table_rates['value'] ) ) : ?>
		...
		<table>
			<thead>
			<tr>
				<th><?php esc_attr_e( 'Region / Country', 'wcvendors-pro' ); ?></th>
			...
			<?php foreach ( $shipping_table_rates['value'] as $rate ) : ?>
				...
				<tr>
					<td width="30%"><?php echo ( esc_attr( $rate['country'] ) != '' ) ? esc_attr( $countries[ strtoupper( $rate['country'] ) ] ) : ( ( esc_attr( $rate['region'] ) != '' ) ? esc_attr( $regions[ strtoupper( $rate['region'] ) ]['name'] ) : __( 'Any', 'wcvendors-pro' ) ); ?></td>
