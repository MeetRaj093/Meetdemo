...
// add_action( 'woocommerce_single_product_summary', 'wcv_show_attribute_links', 5 );

// function wcv_show_attribute_links() {
// 	global $post;
// 	$attribute_names = array( 'author' );
...
//     } elseif ( ! empty( $query->query['pagename'] ) && true === strpos( $query->query['pagename'], '/' ) ) {
//         $query->set( 'post_type', array('product') );
...
//    $columns['pv_shop_name'] = __( 'Vendor'); 
//    return $columns;
// }
...
// 	$store_info = get_store_url( $seller );
// 	$store_name = $store_info['pv_shop_name'];
...
// function wcvendor_store_field($default_uri, $native_slug, $post, $slug, $native_uri) {
...
//         $store_name = get_user_meta($post->post_author, 'pv_shop_name', true);
...
// add_action('woocommerce_single_product_summary', 'store_link_product_page',6);
...
//   echo '<p>Vendor Store: '.$store_link.'</p>';
...
// add_filter( 'woocommerce_loop_product_link', 'filter_function_name_1615', 10, 2 );
// function filter_function_name_1615( $the_permalink, $product ){
// 	global $product;
// 	$vendor_shop 	= urldecode( get_query_var( 'vendor_shop' ) );
//     $vendor_id   	= get_the_author_meta('ID');
//     $store_url      = WCVendors_Pro_Vendor_Controller::get_vendor_store_url( $vendor_id  );
